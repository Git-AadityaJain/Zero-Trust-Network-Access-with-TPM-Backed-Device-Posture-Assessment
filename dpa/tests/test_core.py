...
class TestDPACore(unittest.TestCase):
    ...
    def test_11_re_enrollment(self):
        """Test device re-enrollment (which internally unenrolls)"""
        enrollment = DeviceEnrollment(tpm_exe_path=self.tpm_path)
        
        if not enrollment.is_enrolled():
            self.skipTest("Device not enrolled, cannot test re-enrollment")
        
        # Test internal unenroll method (used during re-enrollment)
        success = enrollment._unenroll_device()
        self.assertTrue(success)
        self.assertFalse(enrollment.is_enrolled())
